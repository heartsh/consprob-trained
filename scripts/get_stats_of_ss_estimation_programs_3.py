...
def main():
  ...
  if not os.path.exists(image_dir_path):
    os.mkdir(image_dir_path)
  seq_lens_avg, seq_nums, consalign_elapsed_times = utils.read_elapsed_time_data(consalign_elapsed_time_data_file_path)
  ...
  data_frame = pandas.DataFrame(data = data)
  num_of_range_few = len(data_frame.query("`Number of RNA sequences` < 6"))
  num_of_range_mid = len(data_frame.query("6 <= `Number of RNA sequences` < 11"))
  ...
  data = {"Average RNA sequence length": seq_lens_avg * 6, "# RNA sequences": seq_nums * 6, "Running time": elapsed_times, "RNA structural aligner": ["ConsAlign"] * len(consalign_elapsed_times) + ["RAF"] * len(raf_elapsed_times) + ["LocARNA"] * len(locarna_elapsed_times) + ["DAFS"] * len(dafs_elapsed_times) + ["SPARSE"] * len(sparse_elapsed_times) + ["LinearTurboFold"] * len(turbofold_elapsed_times)}
  data_frame = pandas.DataFrame(data = data)
  ...
  pyplot.clf()
  ax = seaborn.lmplot(x = "# RNA sequences", y = "Running time", data = data_frame, lowess = True, hue = "RNA structural aligner", scatter = False, legend = False)
  pyplot.tight_layout()
