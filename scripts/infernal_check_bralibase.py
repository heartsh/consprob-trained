...
def main():
  ...
  infernal_black_list_dir_path = asset_dir_path + "/infernal_black_list_bralibase"
  if not os.path.isdir(infernal_black_list_dir_path):
    os.mkdir(infernal_black_list_dir_path)
  temp_dir_path = "/tmp/infernal_check_bralibase_%s" % datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d_%H:%M:%S')
  ...
  for rna_sub_dir in os.listdir(rna_dir_path):
    ...
    for rna_file in os.listdir(rna_align_dir_path):
      ...
      rna_file_path = os.path.join(rna_align_dir_path, rna_file)
      infernal_output_file_path = os.path.join(rna_align_dir_path, rna_name + "_infernal.dat")
