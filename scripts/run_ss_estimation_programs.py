...
def main():
  ...
  mafft_xinsi_params = []
  consalifold_params = []
  consalifold_params_4_running_time = []
  consalifold_old_params = []
  consalifold_old_params_4_running_time = []
  ...
  consalifold_dir_path = asset_dir_path + "/consalifold"
  consalifold_old_dir_path = asset_dir_path + "/consalifold_old"
  ...
  if not os.path.isdir(consalifold_old_dir_path):
    os.mkdir(consalifold_old_dir_path)
  ...
  for rna_file in os.listdir(rna_dir_path):
    ...
    consalifold_output_dir_path = os.path.join(consalifold_dir_path, rna_family_name)
    consalifold_old_output_dir_path = os.path.join(consalifold_old_dir_path, rna_family_name)
    ...
    if not os.path.isdir(consalifold_old_output_dir_path):
      os.mkdir(consalifold_old_output_dir_path)
    ...
    consalifold_params.insert(0, (sub_thread_num, mafft_xinsi_output_file_path, consalifold_output_dir_path, True, False))
    consalifold_params_4_running_time.insert(0, (sub_thread_num, mafft_xinsi_output_file_path, consalifold_output_dir_path, True, True))
    consalifold_old_params.insert(0, (sub_thread_num, mafft_xinsi_output_file_path, consalifold_old_output_dir_path, False, False))
    consalifold_old_params_4_running_time.insert(0, (sub_thread_num, mafft_xinsi_output_file_path, consalifold_old_output_dir_path, False, True))
  ...
  # pool.map(run_consalifold, consalifold_params)
  if False:
    begin = time.time()
    # pool.map(utils.run_command, consalifold_params_4_running_time)
    pool.map(run_consalifold, consalifold_params_4_running_time)
    consalifold_elapsed_time = time.time() - begin
  # pool.map(run_consalifold, consalifold_old_params)
  if False:
    begin = time.time()
    # pool.map(utils.run_command, consalifold_params_4_running_time)
    pool.map(run_consalifold, consalifold_old_params_4_running_time)
    consalifold_old_elapsed_time = time.time() - begin
  pool = multiprocessing.Pool(num_of_threads)
  ...
  if False:
    print("The elapsed time of ConsAlifold (new) = %f [s]." % consalifold_elapsed_time)
    print("The elapsed time of ConsAlifold (old) = %f [s]." % consalifold_old_elapsed_time)
...
def run_consalifold(consalifold_params):
  (sub_thread_num, rna_sa_file_path, consalifold_output_dir_path, is_trained, is_benchmarked) = consalifold_params
  consalifold_command = "consalifold%s %s-t " % ("-trained" if is_trained else "", "-b " if is_benchmarked else "") + str(sub_thread_num) + " -i " + rna_sa_file_path + " -o " + consalifold_output_dir_path
  utils.run_command(consalifold_command)
  for consalifold_output_file in os.listdir(consalifold_output_dir_path):
    if not consalifold_output_file.endswith(".sth"):
      continue
    (body, extension) = os.path.splitext(consalifold_output_file)
    consalifold_output_file_path = os.path.join(consalifold_output_dir_path, consalifold_output_file)
    sta = AlignIO.read(consalifold_output_file_path, "stockholm")
    consalifold_output_file_path_2 = os.path.join(consalifold_output_dir_path, body + ".fa")
    sss = get_sss(sta)
    buf = ""
    consalifold_output_file_2 = open(consalifold_output_file_path_2, "w+")
    for (i, ss) in enumerate(sss):
      buf += ">%d\n%s\n\n" % (i, ss)
    consalifold_output_file_2.write(buf)
    consalifold_output_file_2.close()
